...
Create a copy of any `.example` files and fill them in, removing the `.example` extension. For example, `.env.example` becomes `.env`. The deafaults in the .example are fine for tresting out the project other than a few variables that are mention in the authentication section.
...
There is, however, no need to visit tRPC enpoints manually since you make queries using trpc instead of something like axios.
